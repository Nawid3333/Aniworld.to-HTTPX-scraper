...
from src import genre_stats, term
...
def _print_alert_block(entries, heading, advice):
    ...
    if not entries:
        return
    # Resolved here rather than taken as an argument: every caller wants the
    # host this run actually settled on, and ACTIVE_SITE_URL is only set once
    # domain probing has picked one, so a parameter defaulting to it would
    # freeze whatever the value was at import time.
...
def print_completed_series_alerts(index_manager=None, allow_rescrape=True):
    ...
    try:
        ...
        _print_alert_block(
            completed_not_sub,
            "COMPLETED ANIME — NOT SUBSCRIBED",
            "Consider subscribing or leaving as-is.",
        )
        _print_alert_block(
            started_not_sub,
            "STARTED ANIME — NOT SUBSCRIBED",
            "A started anime is meant to be subscribed. Fix it on the site, then rescrape.",
        )
        ...
        if needs_sub_fix and allow_rescrape:
            ...
            if rescrape == "y":
                ...
                if not urls:
                    ...
                    print(f"\n→ Rescraping {len(urls)} unsubscribed anime...")
                    _run_scrape_and_save(
                        run_kwargs={"url_list": urls, "parallel": False},
                        description=f"Rescrape unsubscribed anime ({len(urls)})",
        ...
        _print_alert_block(
            ongoing_no_wl,
            "ONGOING ANIME — NOT ON WATCHLIST",
...
def _extract_slug_for_suggest(series):
    """Return the comparison key for an index entry, reading link before url.

    The field order matters. show_vanished_series and the index-wide slug set
    it is compared against both read "link" first, so reading "url" first here
    meant an entry whose two fields disagree -- a rename that updated one and
    not the other -- resolved to one slug on this side and a different one on
    the other, and could be called vanished by one and present by the other.
    """
    return _extract_slug_from_field(series.get("link", "") or series.get("url", ""))
...
def _find_vanished_to_clean(idx_mgr=None, ignored=None, seen_slugs=None):
    ...
    Args:
        seen_slugs: slugs the site served during the run that just finished.
            ...
            anime the site just handed us is not vanished, whatever an older
...
def _suggest_something_to_watch(idx_mgr: _IndexLike | None = None):
    ...
    # genre_index.json files each anime under its slug, so join on the slug
    # exactly as genre_stats does. Looking it up by title found genres for 2
    # of 2,490 anime (2026-09-26), so every genre filter came back all but
    # empty and the Genres column read "—" throughout.
    ...
    genre_w = max((len(", ".join(genre_labels.get(g, g) for g in genres) or "—") for _, genres in sample), default=0)
...
def _run_scrape_and_save(
    ...
    try:
        ...
        if scraper is not None and scraper.series_data:
            ...
            if isinstance(result, dict) and result.get("rescrape"):
                remove_series_from_index(SERIES_INDEX_FILE, result["series"])
                for url, title in zip(result["urls"], result["titles"], strict=True):
